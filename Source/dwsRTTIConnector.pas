...
begin
   rttiVariantSym:=TConnectorSymbol.Create(SYS_RTTIVARIANT, TdwsRTTIConnectorType.Create(table));
...
// Create
//
constructor TdwsRTTIConnectorType.Create(table : TSymbolTable);
begin
   FTable := Table;
end;
...
begin
   ...
   if field<>nil then
      ...
      if prop<>nil then begin
         ...
         raise EdwsRTTIException.CreateFmt('"%s" does not hava member "%s" exposed via RTTI',
                                           [instance.FRTTIType.Name, FMemberName]);
      end;
   end;
end;
...
begin
   ...
   if Length(data)<>1 then
      raise EdwsRTTIException.Create('Unsupported Rtti write on thsi data type');
   case VarType(data[0]) of
      varUnknown : begin
         ...
         else raise EdwsRTTIException.Create('Unsupported Rtti write on this datatype');
      end;
   else
