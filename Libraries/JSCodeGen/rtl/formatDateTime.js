function FormatDateTime(fmt, v, u) {
	...
	if (v instanceof Date) {
		...
		if ((u==2) && (Math.abs(v)<1)) v += Math.trunc(new Date().getTime()/864e5+25569);
		dt = new Date(Math.round((v-25569)*864e5));
	}
	if (u==2) dt = new Date(dt.getTime()-dt.getTimezoneOffset()*6e4);
	var r = "";
	for (var i = 0; i < p.length; i++) {
		if (p[i].s) {
			...
			switch (p[i].t) {
				...
				case "ampm": 
					r += ((dt.getUTCHours()||24) <= 12) ? $fmt.TimeAMString : $fmt.TimePMString;
					break; 
				case "am/pm": 
					r += ((dt.getUTCHours()||24) <= 12) ? "am" : "pm";
					break;
				case "a/p": 
					r += ((dt.getUTCHours()||24) <= 12) ? "a" : "p";
					break;
			}
		}
	}
